...
def get_matching_hashes_in_file(
    ...
    for record in screed.open(filename):
        ...
        for kmer_encoded, kmer_in_seq, hashval in get_kmers_for_hashvals(
            ...
            if kmerout_w:
                kmerout_w.writerow(
                    [kmer_in_seq, kmer_encoded, str(hashval), record["name"], filename]
                )
...
def main():
    ...
    # multiply ksize by 3 if using protein signature
    if args.protein or args.dayhoff or args.hp:
        query_ksize = args.ksize * 3
    else:
        query_ksize = args.ksize
    ...
    sigobj = sourmash.load_one_signature(
        args.query, ksize=query_ksize, select_moltype=moltype
    )
